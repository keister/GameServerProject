// 2024082121:08
// 2024082121:08
// 김호준rkaghMy projectAssembly-CSharpInterationView.cs
...
{
    [Header("Circle")] [Range(0, 30)] [SerializeField]
    private float viewRange = 5f; // 시야 범위

    [Range(0, 360)] [SerializeField] private float viewAngle = 360f; // 시야 각도

    [Header("Target")] [SerializeField] private LayerMask targetMask; // 탐색 대상
    [SerializeField] private LayerMask obstacleMask; // 장애물 대상
    [SerializeField] public GameObject target = null;
    [SerializeField] public GameObject prevTarget = null;

    [Header("Draw Line")] [Range(0.1f, 1f)] [SerializeField]
    private float angle = 1f; // 선이 표시될 각도. 작을 수록 선이 촘촘해진다.

    [SerializeField] private List<CastInfo> lineList; // 표시된 선의 정보 리스트
    [SerializeField] private Vector3 offset; // 위치 보정용 벡터. zero 로 해도 무관

    private UI_InteractionGuide _guide;
    ...
    IEnumerator CheckTarget()
    {
        ...
        while (true)
        {
            ...
            if (target != prevTarget)
            {
                if (prevTarget != null)
                {
                    prevTarget.GetComponent<IInteractable>().guide.gameObject.SetActive(false);
                }

                if (curTarget != null)
                {
                    curTarget.GetComponent<IInteractable>().guide.gameObject.SetActive(true);
                }
