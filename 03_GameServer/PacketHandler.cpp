...
namespace game
{
	...
	bool HandlePacket_GameGroup(GameGroup* group, Player& player, Packet pkt)
	{
		PacketType type;
		*pkt >> type;
		...
		case PacketType::C_CHARACTER_RESPAWN:
		{

			group->Handle_C_CHARACTER_RESPAWN(player);
			break;
		}
		default:
			return false;
	...
	Packet Make_S_LEVEL_UP(const int32 level)
	{
		Packet pkt = Packet::Alloc();

		*pkt << PacketType::S_LEVEL_UP;
		*pkt << level;

		return pkt;
	}
	Packet Make_S_CHARACTER_DEATH(const uint64 playerId)
	{
		Packet pkt = Packet::Alloc();

		*pkt << PacketType::S_CHARACTER_DEATH;
		*pkt << playerId;

		return pkt;
	}
	Packet Make_S_CHARACTER_RESPAWN(const uint64 playerId, const int32 hp)
	{
		Packet pkt = Packet::Alloc();

		*pkt << PacketType::S_CHARACTER_RESPAWN;
		*pkt << playerId << hp;
