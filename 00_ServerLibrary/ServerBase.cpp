...
///
//-----------------------------------------------------------------------------
netlib::ServerBase::ServerBase(NetworkAddress addr, int32 numOfWorkerThreads, int32 concurrentThreads, int32 maximumSession)
	...
	, _sessionManager(this, _maxSession)
{
	...
	{
		_threadHandles[threadIdx] = RunThread(&ServerBase::func_worker_thread, this);
	}
	_threadHandles[_numOfWorkerThreads] = RunThread(&ServerBase::func_accept_thread, this);
}
...
///
//-----------------------------------------------------------------------------
bool netlib::ServerBase::Disconnect(uint64 sessionId)
{
...
///
//-----------------------------------------------------------------------------
bool netlib::ServerBase::SendPacket(uint64 sessionId, Packet pkt)
{
	...
	_iocp.Post(0xFFFFFFFF, sessionId, nullptr);
...
void netlib::ServerBase::EnablePacketEncoding(BYTE code, BYTE packetKey)
{
...
///
//-----------------------------------------------------------------------------
bool netlib::ServerBase::send_post(Session& session)
{
...
int32 netlib::ServerBase::recv(Session& session, LPWSABUF wsabuf, DWORD cnt)
{
	DWORD flag = 0;
...
int32 netlib::ServerBase::send(Session& session, LPWSABUF wsabuf, DWORD cnt)
{
...
///
//-----------------------------------------------------------------------------
void netlib::ServerBase::recv_post(Session& session)
{
...
///
//-----------------------------------------------------------------------------
void netlib::ServerBase::handle_send_completion(Session& session, uint32 transferred)
{
	SendBuffer& sendBuf = session.sendBuffer;
...
///
//-----------------------------------------------------------------------------
void netlib::ServerBase::handle_recv_completion(Session& session, uint32 transferred)
{
	RecvBuffer& recvBuf = session.recvBuffer;
...
///
//-----------------------------------------------------------------------------
void netlib::ServerBase::func_worker_thread()
{
...
///
//-----------------------------------------------------------------------------
void netlib::ServerBase::func_accept_thread()
{
	WaitForSingleObject(_evThreadsRun, INFINITE);
	while (true)
	{
		...
		//SocketUtil::SetSendBufferSize(clientSocket, 0); /// 수신버퍼 사이즈 0으로 설정
		//SocketUtil::SetRecvBufferSize(clientSocket, 2048);
